...
fn main() {
    ...
    if args[1] == "-h" {
        println!("Help message: This program plays sounds from a file.\n\
    ...
    if args.len() == 3 && args[2] != "--no-message" {
        ...
        return;
    } else {  println!("This is a numerical quiz game on songs.") }
    ...
    for (index, question) in quiz_data.iter().enumerate() {
        println!("Question {}: Guess the song. Press 'r' to repeat.", index + 1);
        ...
        if trimmed_input == question.correct_answer { 
            println!("Correct!");  
            score += 1;
        } 
        else {
